      ...
      {open && (
        <div className="absolute right-0 mt-2 w-56 rounded-2xl bg-studio-panel border border-studio-border shadow-2xl shadow-black/40 py-1.5 z-50 animate-in fade-in slide-in-from-top-2 duration-150">
          <div className="px-3 py-2 border-b border-studio-border">
            <p className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold">Compte atelier</p>
            <p className="text-xs text-slate-200 truncate mt-0.5">{email}</p>
          </div>
          <Link
            href="/account"
            onClick={() => setOpen(false)}
            className="flex items-center gap-2 px-3 py-2 text-xs text-slate-300 hover:bg-studio-field/60 transition-colors"
          >
