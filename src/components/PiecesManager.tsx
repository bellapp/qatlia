...
const MATERIAL_COLORS: Record<string, string> = {
  ...
  aluminium: 'bg-slate-400/10 text-slate-300 border-slate-500/20',
...
export const PiecesManager: React.FC<PiecesManagerProps> = ({
  ...
  return (
    <div className="space-y-0">
      ...
      <div className="flex items-center justify-between px-1 py-2 gap-2">
        <div className="flex items-center gap-2">
          <div className="relative max-w-[170px] sm:max-w-[220px]">
            ...
            <input
              ...
              className="w-full pl-7 pr-2 py-1.5 rounded-lg bg-studio-field/60 border border-studio-border/80 text-slate-200 placeholder-slate-500 text-xs outline-none focus:border-brand-500/50 transition-colors"
            />
          </div>
          <span className="hidden sm:inline text-[10px] font-mono text-slate-500 tabular-nums">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            className="px-2 py-1 rounded-lg bg-studio-panel hover:bg-studio-field text-slate-400 text-[10px] font-semibold border border-studio-border transition-all"
          >
          ...
          <button
            ...
            className="px-2 py-1 rounded-lg bg-studio-panel hover:bg-studio-field text-slate-400 border border-studio-border transition-all disabled:opacity-30"
            title="CSV"
          >
      ...
      {/* Table Header */}
      <div className="grid grid-cols-12 gap-1.5 px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-slate-500 border-y border-studio-border/50">
                ...
                >
                  ...
                  <div className="col-span-3 sm:col-span-3 flex items-center gap-1 min-w-0">
                    <input
                      ...
                      className="w-full bg-transparent text-slate-200 font-medium text-[11px] outline-none focus:text-white truncate placeholder-slate-600"
                      placeholder="Nom"
                    />
                    {!isFocused && (
                  ...
                  <div className="col-span-3 sm:col-span-3 flex items-center justify-end gap-0.5 font-mono text-[11px] tabular-nums">
                    <input
                      ...
                      className="w-12 sm:w-14 text-right bg-transparent text-slate-100 font-bold outline-none focus:bg-studio-field/60 focus:rounded px-1 py-0.5 -mx-1 tabular-nums"
                        ...
                        <button
                          ...
                          className={`w-5 h-5 rounded font-bold transition-all ${
                            isEdgeActive
                              ...
                              : 'bg-studio-field/60 text-slate-600 hover:text-slate-400 hover:bg-studio-border'
                          }`}
                          title={`Chant ${side}`}
                        >
                          {label}
                  ...
                  <div className="col-span-1 sm:col-span-2 flex items-center justify-end gap-1">
                    <div className="hidden sm:flex text-[9px] font-mono text-slate-500 mr-1 tabular-nums">
      ...
      <div className="sticky bottom-0 -mx-0 pt-2 bg-studio-canvas">
        {showQuickAdd ? (
          <form onSubmit={handleAddPieceQuick} className="p-3 rounded-xl bg-studio-panel/80 border border-brand-500/30 backdrop-blur-sm shadow-lg animate-in slide-in-from-bottom-2 duration-150">
            <div className="grid grid-cols-12 gap-2 items-end">
              <div className="col-span-3">
                <label className="text-[9px] font-semibold uppercase text-slate-500 block mb-0.5">H (cm)</label>
                <input
                  type="number" step="0.1" required autoFocus
                  placeholder="230"
                  value={newHeight}
                  onChange={(e) => setNewHeight(e.target.value)}
                  className="w-full px-2 py-1.5 rounded-lg bg-studio-field border border-studio-border text-slate-100 font-mono text-xs font-bold text-right outline-none focus:border-brand-500/50 placeholder-slate-600 tabular-nums"
                />
              </div>
              <div className="col-span-3">
                <label className="text-[9px] font-semibold uppercase text-slate-500 block mb-0.5">L (cm)</label>
                <input
              ...
              <div className="col-span-2">
                <label className="text-[9px] font-semibold uppercase text-slate-500 block mb-0.5">Qté</label>
                <input
                  type="number" min="1"
                  value={newQty}
              ...
              <div className="col-span-3">
                <label className="text-[9px] font-semibold uppercase text-slate-500 block mb-0.5">Nom</label>
                <input
                  type="text"
                  placeholder="ex: Côté G"
                  value={newReference}
                  onChange={(e) => setNewReference(e.target.value)}
                  className="w-full px-2 py-1.5 rounded-lg bg-studio-field border border-studio-border text-slate-200 text-xs outline-none focus:border-brand-500/50 placeholder-slate-600"
                />
              </div>
            ...
            <div className="flex items-center gap-3 pt-2 mt-2 border-t border-studio-border/60 text-[10px]">
              <span className="font-semibold text-slate-500">Chants :</span>
              {(['left', 'right', 'top', 'bottom'] as const).map((side) => {
                const label = side === 'left' ? 'G' : side === 'right' ? 'D' : side === 'top' ? 'H' : 'B';
                return (
              ...
              <button
                ...
                className="ml-auto text-[10px] text-slate-500 hover:text-slate-300 font-semibold"
              >
                Annuler
              </button>
            </div>
          ...
          <button
            ...
            disabled={disabled}
            className="w-full py-2.5 rounded-xl border-2 border-dashed border-studio-border hover:border-brand-500/40 text-slate-500 hover:text-brand-400 text-xs font-semibold flex items-center justify-center gap-2 transition-all disabled:opacity-30 group cursor-pointer"
          >
